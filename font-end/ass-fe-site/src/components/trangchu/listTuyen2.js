...
function SeatSelection2(props) {
  ...
  const getCurrentPageData = () => {
    const startIndex = (selectedTab === "ngayDi" ? currentPageTab1 : currentPageTab2) * pageSizeTab1 - pageSizeTab1;
    const endIndex = startIndex + (selectedTab === "ngayDi" ? pageSizeTab1 : pageSizeTab2);

    const listChuyenNgayDi = listChuyenReturn1.map(i => {
    ...
    const listChuyenNgayVe = listChuyenReturn2.map(i => {
      const tuyen = listTuyen.find(t => i.tuyenXe === t.maTuyenXe);
      return {
        ...i,
        tuyenXe: tuyen
    ...
    const selectedList = selectedTab === "ngayDi" ? listChuyenNgayDi : listChuyenNgayVe;

    return selectedList.slice(startIndex, endIndex);
  };
  ...
  const formattedNgayDi = ngayDi ? format(parseISO(ngayDi), 'dd-MM', { locale: viLocale }) : '';
  const dayOfWeek = ngayDi ? format(parseISO(ngayDi), 'EEEE', { locale: viLocale }) : '';

  const formattedNgayVe = ngayVe ? format(parseISO(ngayVe), 'dd-MM', { locale: viLocale }) : '';
  const dayOfWeek2 = ngayVe ? format(parseISO(ngayVe), 'EEEE', { locale: viLocale }) : '';
  ...
  return (
    <div className="grid-listtuyen-container-loc">
      ...
      <Tabs
        ...
        className="TabKhuhoi"
      >
        <TabPane tab={`Chuyến Đi - ${dayOfWeek}, ${formattedNgayDi}`} key="ngayDi">
        ...
        <TabPane tab={`Chuyến Về - ${dayOfWeek2}, ${formattedNgayVe}`} key="ngayVe">
      ...
      {areBothChuyensSelected() && (
        <Drawer
          ...
          size="large"
        >
           <DatVeForm chuyenTab1={selectedChuyenTab1} chuyenTab2={selectedChuyenTab2} onClose={onClose} />
